...
class Network:
    ...
    def run_distance_vector(self, max_iterations=100):
        """Run the distance vector algorithm until convergence or max iterations"""
        time_step = 0
        ...
        while any_change and time_step < max_iterations:
            ...
            for name, router in self.routers.items():
                advertisements[name] = router.get_distance_vector()                        
            for router_name, router in self.routers.items():
                for neighbor_name in router.neighbors:
                    if neighbor_name in advertisements:
                        neighbor_cost = router.neighbors[neighbor_name]
                        neighbor_dv = advertisements[neighbor_name]                                                
    ...
    def process_updates(self, updates, start_time_step):
        """Process link updates and continue distance vector algorithm"""
        if not updates:
